...
if selected_cities:
    ...
    fig_cities.update_layout(
        ...
        yaxis2=dict(
            title='為替レート (USD/JPY)',
            overlaying='y',  # y軸を重ねて表示
            side='right',
            showgrid=show_exchange_rate  # 為替レートの軸表示を制御
        ),
        yaxis3=dict(
            title='WTI原油価格',
            overlaying='y',  # y軸を重ねて表示
            side='right',  # WTI原油価格の軸を右側に配置
            position=1.0,  # 値を1.0に修正
            showgrid=show_wti  # WTI原油価格の軸表示を制御
        ),
        hovermode='x unified',
        legend=dict(
            x=1.2,  # 凡例の位置
